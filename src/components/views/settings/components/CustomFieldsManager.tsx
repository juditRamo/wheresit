...
export function CustomFieldsManager() {
  ...
  const { fields, createField, updateField, deleteField, reorderFields, createOption, updateOption, deleteOption } = useCustomFieldsContext()
  ...
  async function handleCreate() {
    ...
    await createField({ name: newName.trim(), field_type: newType })
    resetCreate()
  }
                ...
                <div className="cf-manager__options">
                  ...
                  {field.options.map(opt => (
                    <InlineOptionEditor
                      key={opt.id}
                      option={opt}
                      onUpdate={label => updateOption(opt.id, label)}
                      onDelete={() => deleteOption(opt.id)}
                    />
                  ))}
                  <TrailingOptionInput
                    onAdd={label => createOption(field.id, label)}
